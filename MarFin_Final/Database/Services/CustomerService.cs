...
namespace MarFin_Final.Data
{
    public class CustomerService
    {
            ...
            try
            {
                using (SqlConnection conn = DBConnection.GetConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Country", customer.Country ?? "Philippines");
                        cmd.Parameters.AddWithValue("@CustomerStatus", customer.CustomerStatus ?? "Lead");
        ...
        // OPTIMIZED - Search customers with lazy loading (max 20 results)
        public async Task<List<Customer>> SearchCustomersAsync(string searchTerm, int maxResults = 20)
        {
                ...
                using (SqlConnection conn = DBConnection.GetConnection())
                {
                    ...
                    ORDER BY 
                        CASE 
                            ...
                            WHEN company_name LIKE '%' + @Search + '%' THEN 2
                            ELSE 3 
                        END,
